...
async function boot() {
  ...
  // Hide new matter button for non-attorneys
  if (d.actor.role !== 'attorney') { 
...
async function selectMatter(id) { 
  ...
  // Hide create task button for non-attorneys
  if (state.actor.role !== 'attorney') { 
...
$('#matter-form').onsubmit = async e => { 
  ...
  try { 
    const d = await api('/api/matters', { method: 'POST', body: JSON.stringify({ title: f.get('title'), description: f.get('description'), privilegeLabel: f.has('privilegeLabel'), workProductLabel: f.has('workProductLabel'), selfAnalysisLabel: f.has('selfAnalysisLabel'), memberIds: f.get('memberId') ? [f.get('memberId')] : [] }) }); 
    $('#matter-dialog').close(); 
    e.target.reset(); 
    state.selected = d.id; 
    await boot() 
...
$('#task-form').onsubmit = async e => { 
  ...
  try { 
    await api(`/api/matters/${state.selected}/tasks`, { method: 'POST', body: JSON.stringify({ title: f.get('title'), instructions: f.get('instructions'), assigneeId: f.get('assigneeId'), dueAt: f.get('dueAt') || null }) }); 
    $('#task-dialog').close(); 
    e.target.reset(); 
    await selectMatter(state.selected) 
  } catch (x) { 
...
$('#message-form').onsubmit = async e => { 
  ...
  const f = new FormData(e.target); 
  try { 
    await api(`/api/matters/${state.selected}/messages`, { method: 'POST', body: JSON.stringify({ body: f.get('body') }) }); 
    e.target.reset(); 
    await selectMatter(state.selected) 
  } catch (x) { 
    alert(x.message) 
